...
[ "$install_root" != "" ] && {
  mkdir -p "$install_root"/usr/{bin,share/{applications,pixmaps,whats/{utils,icon_status}},doc/whats-"$pkgver"}
} || {
  mkdir -p /usr/{share/whats/{utils,icon_status},doc/whats-"$pkgver"}
}
...
install -Dm 0755 utils/* "$install_root"/usr/share/whats/utils
